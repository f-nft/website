...
class App extends Component {
	...
	render() {
		...
		async function connectWallet() {
			...
			var getstakednfts = await vaultcontract.methods.tokensOfOwner(account).call().catch((err) => alert(err, 'Please Connect To Polygon Network'));

			document.getElementById('yournfts').textContent = getstakednfts;
			var getbalance = Number(await vaultcontract.methods.balanceOf(account).call().catch((err) => alert(err)));

			document.getElementById('stakedbalance').textContent = getbalance;
			var arraynft = []
			getstakednfts ?

				(arraynft = Array.from(getstakednfts.map(Number))) : (arraynft = []);
			const tokenid = arraynft.filter(Number);
			var rwdArray = [];
			tokenid.forEach(async (id) => {
				var rawearn = await vaultcontract.methods.earningInfo(account, [id]).call().catch((err) => alert(err));
		...
		return (
			<div className="items-center justify-start p-2 text-center">
				<nav className='navbar full-width navbar-expand-md navbar-dark mb-3'>
					...
					<div className='px-5'>
						<input id='connectbtn' type='Button' className=' connectbutton font-blk' onClick={connectWallet} style={{ fontSize: '20px', border: '0.2px', borderRadius: '15px', boxShadow: '1px 1px 5px #000000', fontFamily: 'Rambla' }} value='Connect Your Wallet' />
					</div>
				</nav>
				<div id='nftminter' className='flex-1 justify-between items-center p-5'>
					<div className='nftminted row p-3 center'>
						<div className='col'>
							<img src='f-nft0-100.gif' width='79%' alt='fantasy' style={{ border: '2px', borderRadius: '15px' }} />
							<div>
						...
						<div className='col justify-center'>
							<div className='row container-fluid center p-2'>
								...
								<div id='wallet-address' style={{
									...
									<label for='floatingInput'>Please <input id='connectbtn' type='Button' className='stakedcard' onClick={connectWallet} style={{ fontSize: '20px', border: '0.2px', borderRadius: '15px', boxShadow: '1px 1px 5px #000000', fontFamily: 'Rambla' }} value='Connect Wallet' /></label>
								</div>
							</div>
							<div>
							...
							<h6 className='pt-1' style={{ fontFamily: 'Rambla', fontWeight: '300', fontSize: '18px', marginBottom: '1px', textShadow: '1px 1px 4px #fffff0' }}>CRYPTO PREFER TO PAY</h6>
							<div className='row row-style' style={{ color: '#000039', marginTop: '1px', fontFamily: 'Rambla', fontSize: '10px', textShadow: '0px 1px 2px #fffff0' }}>
								...
								<div>
									<label id='txout pb-2' style={{ color: '#399F14', marginTop: '5px', fontWeight: '500', textShadow: '1px 1px 2px #000000' }}>
									</label>
								</div>
							</div>
						</div>
						...
						<div className="d-flex justify-content-around">
							<div className='align-self-start stakingrewards flow-row flex-1 basis-1 items-center p-3' style={{ borderRadius: '25px', boxShadow: '1px 1px 15px #ffffff', fontFamily: 'Rambla', minWidth: '250px', maxWidth: '250px', maxHeight: '300px', minHeight: '300px' }}>
								...
								<table className='table mt-3 mb-5 px-3 table-dark'>
									<tr>
										<td style={{ fontSize: '16px' }}>Your NFT:
									...
									<tr>
										<td style={{ fontSize: '16px' }}>Staked NFT:
							...
							<div className=' align-self-end stakingrewards flex-1 basis-1 items-center p-3' style={{ borderRadius: '25px', boxShadow: '1px 1px 15px #ffffff', fontFamily: 'Rambla', minWidth: '250px', maxWidth: '250px', maxHeight: '300px', minHeight: '300px' }}>
								...
								<Button onClick={rewardinfo} style={{ backgroundColor: '#ffffff10', boxShadow: '1px 1px 5px #000000' }} >Earn FOT Rewards</Button>
								<div id='earned' style={{ color: '#f83dd9', marginTop: '5px', fontSize: '25px', fontWeight: '500', textShadow: '1px 1px 2px #000000' }}><p style={{ fontSize: '20px' }}>Tokens Earned...</p></div>
						...
						<div className="grid grid-flow-row-dense items-center grid-cols-4 grid-rows-3 ...">
							<div className="col-span-2 row-span-4 p-2">
								<div className='flex-non justify-center items-center pb-1' >
									<img className='justify-start react-logo' src='FNFT.png' alt='logo' />
									<div id='listnft' className='row'>
										<div id='titlelist' className='row center'>
											<h2 className='row center' style={{ color: 'white', border: '1px', paddingInline: '1px', borderRadius: '5px', boxShadow: '1px 1px 5px #000000' }} >VAULT</h2>
